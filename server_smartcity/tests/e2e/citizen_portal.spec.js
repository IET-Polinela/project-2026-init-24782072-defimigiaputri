...
//   Opsi 1 (Live Server): 'http://127.0.0.1:5500/smartcity_citizen_spa_24782072/index.html'
//   Opsi 2 (http-server):  'http://localhost:8080/index.html'
//   Opsi 3 (file://):      'file:///C:/Users/.../smartcity_citizen_spa_24782072/index.html'
...
// ---------------------------------------------------------------------------
const TEST_CITIZEN_USERNAME = 'putri';
const TEST_CITIZEN_PASSWORD = 'putri123';
const TEST_ADMIN_USERNAME  = 'defii';
const TEST_ADMIN_PASSWORD  = 'defi123';
...
test.describe('Modul 5: Interaktivitas UI (UI-01 through UI-06)', () => {
    ...
    test('UI-02: Live Search pada daftar laporan admin berfungsi', async ({ page }) => {
        ...
        // Tunggu sebentar agar AJAX selesai
        await page.waitForTimeout(800);

        // Ambil jumlah baris setelah filter
        const filteredRowCount =
            await tableBody.locator('tr').count();

        console.log(
            `[UI-02] Jumlah baris setelah filter: ${filteredRowCount}`
        );

        // Minimal harus ada hasil
        expect(filteredRowCount).toBeGreaterThan(0);

        console.log(
            "[UI-02] ✅ Live search berfungsi: input → AJAX → tabel terupdate"
        );
    });
    ...
    test('UI-03: Pagination Feed Kota — maks 10 kartu, kontrol pagination muncul', async ({ page }) => {
        ...
        for (let i = 1; i <= 25; i++) {
            mockReports.push({
                ...
                category: i % 2 === 0 ? 'Infrastruktur Jalan' : 'Kebersihan',
        ...
        // Feed Kota langsung tampil pada dashboard (tidak memakai tab lagi)
        const feedContainer = page.locator('#feedContainer');
        await expect(feedContainer).toBeVisible();

        // -------------------------------------------------------------------
        // LANGKAH 5: Hitung jumlah kartu laporan
        // -------------------------------------------------------------------
        const reportCards = feedContainer.locator('.card');
        ...
        console.log(
            `[UI-03] Jumlah kartu Feed Kota: ${cardCount} (maks 10)`
        );

        // -------------------------------------------------------------------
        // LANGKAH 6: Verifikasi pagination
        // -------------------------------------------------------------------
        const paginationContainer =
            page.locator('#feedPaginationContainer');

        await expect(paginationContainer).toBeVisible();

        // Minimal harus ada tulisan "Halaman"
        await expect(paginationContainer).toContainText("Halaman");

        console.log(
            `[UI-03] ✅ Pagination terverifikasi`
        );
    });
    ...
    test('UI-05: Isi form dan simpan draft → modal tutup, notifikasi muncul', async ({ page }) => {
        ...
        await expect(btnBukaModal).toBeVisible();
        ...
        await page.locator('#inputCategory').selectOption('Infrastruktur Jalan');
